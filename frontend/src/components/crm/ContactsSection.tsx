            ...
            <div className="divide-y divide-border">
              {contacts.map((c) => (
                <div key={c.id} className="flex items-start justify-between py-2 gap-3">
                  ...
                  <button onClick={() => handleDelete(c.id)} className="text-muted-foreground hover:text-red-500 flex-shrink-0">
...
function Input({ label, value, onChange, type = "text", required = false }: {
  ...
  return (
    <div>
      <label className="block text-xs text-muted-foreground mb-0.5" htmlFor={id}>{label}</label>
      <input
        id={id} type={type} value={value} onChange={(e) => onChange(e.target.value)} required={required}
        className="w-full px-2 py-1.5 text-xs border border-border rounded bg-card focus:outline-none focus-visible:ring-1 focus-visible:ring-ring"
      />
    </div>
  );
}
