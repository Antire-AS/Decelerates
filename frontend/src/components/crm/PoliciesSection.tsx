...
// Product types are used as enum values sent to the backend — keep Norwegian canon
const PRODUCT_TYPES = [
...
function DaysBadge({ renewalDate }: { renewalDate?: string }) {
  ...
  if (days < 0)  return <span className="text-xs text-red-600">🔴 {T("Utløpt")} ({Math.abs(days)}{T("d")})</span>;
  if (days <= 30) return <span className="text-xs text-orange-600">🟠 {days} {T("dager")}</span>;
  if (days <= 90) return <span className="text-xs text-yellow-600">🟡 {days} {T("dager")}</span>;
  return <span className="text-xs text-green-700">🟢 {days} {T("dager")}</span>;
}
    ...
    <div className="space-y-2">
      ...
      <div className="broker-card">
        <button onClick={() => setListOpen((o) => !o)}
          ...
          <span>📋 {T("Forsikringsavtaler")} {policies.length > 0 && `(${policies.length})`}</span>
          {listOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
        {listOpen && (
          <div className="mt-3">
            {policies.length === 0 ? (
              <p className="text-xs text-muted-foreground">{T("Ingen forsikringsavtaler registrert.")}</p>
            ) : (
                  ...
                  <div key={p.id} className="py-2.5 flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {[
                          p.policy_number && `${T("Avtalenr")}: ${p.policy_number}`,
                          p.annual_premium_nok && `${T("Premie")}: ${fmtNok(p.annual_premium_nok)}`,
                          p.commission_rate_pct != null && `${T("Provisjon")}: ${p.commission_rate_pct}%`,
                        ].filter(Boolean).join(" · ")}
                      </p>
                      {p.document_url && (
                        <a href={p.document_url} target="_blank" rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 text-xs text-primary hover:underline mt-0.5">
                          📄 {T("Avtaledokument")} <ExternalLink className="w-3 h-3" />
                        </a>
                      )}
                      {p.notes && (
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <DaysBadge renewalDate={p.renewal_date} />
      ...
      <div className="broker-card">
        <button onClick={() => setFormOpen((o) => !o)}
          className="w-full flex items-center justify-between text-sm font-semibold text-foreground">
          <span className="flex items-center gap-1.5"><Plus className="w-4 h-4" /> {T("Registrer forsikringsavtale")}</span>
        ...
        {formOpen && (
          <form onSubmit={handleAdd} className="mt-3 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label-xs" htmlFor="policy-insurer">{T("Forsikringsselskap")} *</label>
                <input id="policy-insurer" value={insurer} onChange={(e) => setInsurer(e.target.value)} required
                  className="input-sm w-full" />
              </div>
              <div>
                <label className="label-xs" htmlFor="policy-product-type">{T("Produkttype")} *</label>
              ...
              <div>
                <label className="label-xs" htmlFor="policy-number">{T("Avtalenummer")}</label>
                <input id="policy-number" value={policyNumber} onChange={(e) => setPolicyNumber(e.target.value)} className="input-sm w-full" />
              </div>
              <div>
                <label className="label-xs" htmlFor="policy-status">{T("Status")}</label>
              ...
              <div>
                <label className="label-xs" htmlFor="policy-premium">{T("Årspremie (kr)")}</label>
              ...
              <div>
                <label className="label-xs" htmlFor="policy-coverage">{T("Forsikringssum (kr)")}</label>
              ...
              <div>
                <label className="label-xs" htmlFor="policy-start-date">{T("Startdato")}</label>
                <input id="policy-start-date" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="input-sm w-full" />
              </div>
              <div>
                <label className="label-xs" htmlFor="policy-renewal-date">{T("Fornyelsesdato")}</label>
              ...
              <div>
                <label className="label-xs" htmlFor="policy-commission-rate">{T("Provisjonssats (%)")}</label>
                <input id="policy-commission-rate" type="number" value={commissionRate} onChange={(e) => setCommissionRate(e.target.value)}
                  min="0" max="100" step="0.1" placeholder={T("f.eks. 12.5")} className="input-sm w-full" />
              </div>
              <div>
                <label className="label-xs" htmlFor="policy-commission-amount">{T("Provisjonsbeløp (kr)")}</label>
                <input id="policy-commission-amount" type="number" value={commissionAmt} onChange={(e) => setCommissionAmt(e.target.value)}
                  min="0" step="100" placeholder={T("Beregnes automatisk om tomt")} className="input-sm w-full" />
              </div>
            </div>
            <div>
              <label className="label-xs" htmlFor="policy-doc-url">{T("Dokument-URL")}</label>
            ...
            <div>
              <label className="label-xs" htmlFor="policy-notes">{T("Notater")}</label>
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={saving}
                ...
                {saving ? T("Lagrer…") : T("Lagre avtale")}
              </button>
