...
export default function ActivitiesSection({ orgnr }: { orgnr: string }) {
  ...
  const TYPE_LABEL: Record<string, string> = {
    call: T("Samtale"), email: T("E-post"), meeting: T("Møte"), note: T("Notat"), task: T("Oppgave"),
  };
    ...
    <div className="space-y-2">
      <div className="broker-card">
        <button onClick={() => setListOpen((o) => !o)}
          ...
          <span>📅 {T("Aktiviteter")} {activities.length > 0 && `(${activities.length})`}</span>
          {listOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
        {listOpen && (
          <div className="mt-3">
            {activities.length === 0 ? (
              <p className="text-xs text-muted-foreground">{T("Ingen aktiviteter registrert.")}</p>
            ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {a.due_date && !a.completed && (
                        <p className="text-xs text-orange-600 flex items-center gap-1 mt-0.5">
                          <Clock className="w-3 h-3" /> {T("Frist")}: {a.due_date}
                        </p>
                      )}
                      {a.assigned_to_user_id != null && userById.has(a.assigned_to_user_id) && (
                        <p className="text-xs text-primary flex items-center gap-1 mt-0.5">
                          <UserIcon className="w-3 h-3" />
                          {T("Tildelt")}: {userById.get(a.assigned_to_user_id)?.name || userById.get(a.assigned_to_user_id)?.email}
                        </p>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {!a.completed && a.activity_type === "task" && (
                        <button onClick={() => handleComplete(a.id)}
                          className="text-muted-foreground hover:text-green-600" title={T("Merk fullført")}>
      ...
      <div className="broker-card">
        <button onClick={() => setFormOpen((o) => !o)}
          className="w-full flex items-center justify-between text-sm font-semibold text-foreground">
          <span className="flex items-center gap-1.5"><Plus className="w-4 h-4" /> {T("Legg til aktivitet")}</span>
        ...
        {formOpen && (
          <form onSubmit={handleAdd} className="mt-3 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="label-xs" htmlFor="activity-type">{T("Type")}</label>
              ...
              <div>
                <label className="label-xs" htmlFor="activity-subject">{T("Emne")} *</label>
                <input id="activity-subject" value={subject} onChange={(e) => setSubject(e.target.value)} required className="input-sm w-full" />
              </div>
            </div>
            <div>
              <label className="label-xs" htmlFor="activity-body">{T("Detaljer")}</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="label-xs" htmlFor="activity-due-date">{T("Frist (kun for oppgaver)")}</label>
                <input id="activity-due-date" type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="input-sm w-full" />
              </div>
              <div>
                <label className="label-xs" htmlFor="activity-assigned-to">{T("Tildelt")}</label>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button type="submit" disabled={saving}
                ...
                {saving ? T("Lagrer…") : T("Legg til aktivitet")}
              </button>
              <button type="button" onClick={() => setFormOpen(false)}
                className="w-full sm:w-auto px-3 py-2 sm:py-1.5 text-xs rounded border border-border text-muted-foreground">{T("Avbryt")}</button>
            </div>
          </form>
        )}
      </div>
