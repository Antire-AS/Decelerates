...
export default function ClaimsSection({ orgnr, policies }: {
  ...
  const STATUS_LABEL: Record<string, string> = {
    open: T("Åpen"), in_review: T("Under behandling"), settled: T("Avgjort"), rejected: T("Avvist"),
  };
    ...
    <div className="space-y-2">
      <div className="broker-card">
        <button onClick={() => setListOpen((o) => !o)}
          ...
          <span>🔥 {T("Skader og krav")} {claims.length > 0 && `(${claims.length})`}</span>
          {listOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
        {listOpen && (
          <div className="mt-3">
            {claims.length === 0 ? (
              <p className="text-xs text-muted-foreground">{T("Ingen registrerte skader.")}</p>
            ) : (
                  ...
                  <div key={c.id} className="py-2.5 flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-foreground">
                        {c.claim_number || `${T("Skade")} #${c.id}`}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {[
                          c.incident_date && `${T("Hendelse")}: ${c.incident_date}`,
                          c.estimated_amount_nok && `${T("Estimert")}: ${fmtNok(c.estimated_amount_nok)}`,
                          c.settled_amount_nok && `${T("Oppgjort")}: ${fmtNok(c.settled_amount_nok)}`,
                        ].filter(Boolean).join(" · ")}
                      </p>
                      {c.description && (
      ...
      <div className="broker-card">
        <button onClick={() => setFormOpen((o) => !o)}
          className="w-full flex items-center justify-between text-sm font-semibold text-foreground">
          <span className="flex items-center gap-1.5"><Plus className="w-4 h-4" /> {T("Registrer skade")}</span>
        ...
        {formOpen && (
          <form onSubmit={handleAdd} className="mt-3 space-y-3">
            {policies.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {T("Ingen forsikringsavtaler å knytte skaden til. Registrer en avtale først.")}
              </p>
            ) : (
              <>
                <div>
                  <label className="label-xs" htmlFor="claim-policy">{T("Forsikringsavtale")} *</label>
                  <select id="claim-policy" value={policyId} onChange={(e) => setPolicyId(e.target.value)} className="input-sm w-full" required>
                    <option value="">{T("Velg avtale…")}</option>
                    {policies.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.insurer} — {p.product_type} ({p.policy_number || T("uten nr")})
                      </option>
                    ))}
                  </select>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="label-xs" htmlFor="claim-reported-date">{T("Meldt dato")}</label>
                  ...
                  <div>
                    <label className="label-xs" htmlFor="claim-estimated-amount">{T("Estimert beløp (kr)")}</label>
                  ...
                  <div>
                    <label className="label-xs" htmlFor="claim-insurer-contact">{T("Kontakt hos forsikringsselskap")}</label>
                    <input id="claim-insurer-contact" value={insContact} onChange={(e) => setInsContact(e.target.value)} className="input-sm w-full" />
                  </div>
                </div>
                <div>
                  <label className="label-xs" htmlFor="claim-description">{T("Beskrivelse")}</label>
                ...
                <div>
                  <label className="label-xs" htmlFor="claim-notes">{T("Notater")}</label>
                  <textarea id="claim-notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={2}
                    placeholder={T("Interne notater (synlig kun for megler)")}
                    className="w-full px-2 py-1.5 text-xs border border-border rounded-lg bg-card resize-none focus:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
                </div>
                ...
                <div className="flex gap-2">
                  <button type="submit" disabled={saving}
                    ...
                    {saving ? T("Registrerer…") : T("Registrer skade")}
                  </button>
