    ...
    <div className="space-y-4">
      ...
      <div className="broker-card space-y-3">
        ...
        <textarea
          ...
          rows={3}
          className="w-full text-sm border border-border rounded-lg px-3 py-2 resize-none focus:outline-none focus-visible:ring-1 focus-visible:ring-ring text-foreground placeholder:text-muted-foreground"
        />
      ...
      <div className="broker-card space-y-1">
        ...
        {isLoading ? (
          ...
          <p className="text-xs text-muted-foreground">Ingen notater ennå.</p>
        ) : (
          <div className="divide-y divide-border">
            {notes.map((n) => (
              <div key={n.id} className="py-3 flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground whitespace-pre-wrap break-words">{n.text}</p>
                  <p className="text-xs text-muted-foreground mt-1">{n.created_at.slice(0, 10)}</p>
                </div>
                <button
                  onClick={() => handleDelete(n.id)}
                  className="flex-shrink-0 text-muted-foreground hover:text-red-500 mt-0.5"
                >
