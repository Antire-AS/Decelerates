...
export default function StrukturSection({ struktur }: Props) {
  ...
  return (
    <Section title={T("Konsernstruktur")}>
      {hasParent && (
        <div className="rounded-lg bg-accent border border-border px-3 py-2 mb-3">
          <p className="text-[10px] font-bold uppercase tracking-wide text-primary mb-1">{T("Morselskap")}</p>
      ...
      {subUnits.length > 0 && (
        <div>
          ...
          <div className="space-y-1">
            ...
            {totalSubs > 8 && (
              <p className="text-xs text-muted-foreground">+ {totalSubs - 8} {T("flere")}</p>
            )}
          </div>
        </div>
      )}
