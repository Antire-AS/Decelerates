          ...
          {([
            ...
            <button
              ...
              className={`px-3 py-1 text-xs rounded-lg font-medium transition-colors ${
                filter === id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted"
              }`}
            >
              {label}
            </button>
            ...
            {filteredResults.map((r, i) => (
              <div key={i} className="broker-card space-y-2">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-muted text-muted-foreground truncate max-w-[60%]">
                ...
                <p className="text-sm text-foreground leading-relaxed line-clamp-5">{r.chunk_text}</p>
              </div>
            ))}
          </div>
        )
