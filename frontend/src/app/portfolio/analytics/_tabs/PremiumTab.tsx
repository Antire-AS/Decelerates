...
export default function PremiumTab() {
  ...
  if (error || !data) return <div className="broker-card text-center py-10 text-sm text-muted-foreground">Ingen policyer registrert ennå. Legg til policyer i Selskapsøk → CRM-fanen.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {insurerData.length > 0 && (
          <div className="broker-card">
            <h3 className="text-sm font-semibold text-foreground mb-4">Premie per forsikringsselskap</h3>
            ...
            <div className="mt-3 space-y-1.5">
              {insurerData.map((d, i) => (
                <div key={d.name} className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-muted-foreground">{d.count} avtaler · {d.pct.toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {productData.length > 0 && (
          <div className="broker-card">
            <h3 className="text-sm font-semibold text-foreground mb-4">Premie per produkttype</h3>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
      ...
      {(data.by_status ?? []).length > 0 && (
        <div className="broker-card">
          <h3 className="text-sm font-semibold text-foreground mb-3">Fordeling per status</h3>
