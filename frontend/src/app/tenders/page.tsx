      ...
      <div className="flex-1 min-w-0">
        ...
        {showNew && (
          <NewTenderModal
            ...
            onCreated={() => { setShowNew(false); mutate(); }}
          />
        )}
        ...
        <ConfirmDialog
          ...
          destructive
          onConfirm={() => { if (deleteId !== null) deleteTender(deleteId).then(() => mutate()); }}
        />
      </div>
...
function NewTenderModal({
  ...
  const { data: catalog } = useSWR<InsuranceProductOut[]>("insurance-products", () => getInsuranceProducts());
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-card rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          <ProductPicker catalog={catalog} selected={products} onToggle={toggleProduct} T={T} />
          <div>
          ...
          <div>
              ...
              <div className="flex flex-wrap gap-2 mt-1 mb-2">
                {recipients.map((r) => (
                  <span key={r.insurer_name} className="flex items-center gap-1 text-xs bg-accent text-foreground px-2 py-1 rounded-full">
                    {r.insurer_name}
                    <button onClick={() => removeRecipient(r.insurer_name)}><X className="w-3 h-3" /></button>
                  </span>
                ))}
              </div>
            )}
            <div className="flex flex-wrap gap-2 mt-1">
              {insurers.filter((i) => !recipients.find((r) => r.insurer_name === i.name)).map((ins) => (
                <button key={ins.id} onClick={() => addInsurer(ins)} className="text-xs px-2.5 py-1 rounded-full border border-border text-muted-foreground hover:border-primary hover:text-primary">
                  + {ins.name}
                </button>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleCreate} disabled={!orgnr || !title || !products.length || saving} className="px-6 py-2 bg-primary text-primary-foreground text-sm rounded-lg hover:bg-primary/80 disabled:opacity-50">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          {FALLBACK_PRODUCTS.map((p) => (
            <button key={p} onClick={() => onToggle(p)} className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${selected.includes(p) ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border hover:border-primary"}`}>
              {T(p)}
            </button>
          ))}
        </div>
    ...
    <div>
      ...
      <div className="space-y-3 mt-1">
        {Object.keys(groups).map((cat) => (
          <div key={cat}>
            <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground mb-1">{T(CATEGORY_LABEL[cat] ?? cat)}</p>
            <div className="flex flex-wrap gap-2">
              {groups[cat].map((p) => (
                <button key={p.id} onClick={() => onToggle(p.name)} title={p.description ?? undefined} className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${selected.includes(p.name) ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border hover:border-primary"}`}>
                  {p.name}
                </button>
              ))}
            </div>
