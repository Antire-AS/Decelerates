  ...
  if (error || !data) return (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="broker-card text-center max-w-sm mx-4">
        ...
        <p className="text-sm font-semibold text-foreground">Lenken er ugyldig eller utløpt</p>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        {riskScore != null && (
          <div className="broker-card">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-foreground">Risikoscore</p>
              <span className={`text-sm font-bold px-3 py-1 rounded-full ${
                bandTailwindClass(bandFor(riskScore).label)
              }`}>
                {riskScore} / 20
              </span>
            </div>
            {(data.risk_reasons ?? []).length > 0 && (
              <ul className="mt-2 space-y-1">
                {data.risk_reasons!.map((r, i) => (
                  <li key={i} className="text-xs text-muted-foreground flex gap-2"><span>•</span>{r}</li>
                ))}
              </ul>
            )}
          </div>
            ...
            <div className="space-y-2">
              {data.policies.map((p, i) => (
                <div key={i} className="flex items-start justify-between text-sm py-2 border-b border-border last:border-0">
                  <div>
                    <p className="font-medium text-foreground">{p.product_type}</p>
        ...
        {data.claims.length > 0 && (
          <div className="broker-card">
            <h2 className="text-sm font-semibold text-foreground mb-3">Skader</h2>
            <div className="space-y-2">
              {data.claims.map((c, i) => (
                <div key={i} className="flex items-start justify-between text-sm py-1.5 border-b border-border last:border-0">
                  <div>
                    <p className="text-foreground">{c.description ?? `Skade #${i + 1}`}</p>
                    {c.incident_date && (
                      <p className="text-xs text-muted-foreground">
        ...
        {data.documents.length > 0 && (
          <div className="broker-card">
            <h2 className="text-sm font-semibold text-foreground mb-3">Dokumenter</h2>
        ...
        {data.policies.length === 0 && data.claims.length === 0 && data.documents.length === 0 && (
          <div className="broker-card text-center py-10">
            ...
            <p className="text-sm text-muted-foreground">Ingen forsikringer registrert ennå.</p>
          </div>
        )}
