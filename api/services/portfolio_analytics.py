...
class PortfolioAnalyticsService:
    ...
    def get_analytics(self, portfolio_id: int, firm_id: int, get_portfolio_fn) -> dict:
        ...
        get_portfolio_fn(portfolio_id, firm_id)  # raises NotFoundError if missing or wrong firm
        orgnrs = [
            pc.orgnr for pc in
...
# Backward compat
def get_analytics(portfolio_id: int, firm_id: int, db: Session, get_portfolio_fn) -> dict:
    return PortfolioAnalyticsService(db).get_analytics(portfolio_id, firm_id, get_portfolio_fn)


def get_concentration(portfolio_id: int, db: Session, get_portfolio_fn) -> dict:
    return PortfolioAnalyticsService(db).get_concentration(portfolio_id, get_portfolio_fn)
