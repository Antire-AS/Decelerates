...
_COMPANIES = [
    ...
    {
        "orgnr": "999100102",
        "navn": "Nordvind Teknologi AS",
        "risk_score": 45,
        "naeringskode1": "62.010",
        "naeringskode1_beskrivelse": "Programmeringsvirksomhet",
        "kommune": "Bergen",
        "antall_ansatte": 35,
        "base_revenue": 28_000_000,
        "base_result": 3_200_000,
        "base_assets": 22_000_000,
        "base_equity": 14_000_000,
    ...
    {
        "orgnr": "999100103",
        "navn": "Fjordbygg Entreprenør AS",
        "risk_score": 72,
        "naeringskode1": "41.200",
        "naeringskode1_beskrivelse": "Oppføring av bygninger",
        "kommune": "Stavanger",
        "antall_ansatte": 52,
        "base_revenue": 95_000_000,
        "base_result": -2_800_000,
        "base_assets": 65_000_000,
        "base_equity": 12_000_000,
    ...
    {
        "orgnr": "999100104",
        "navn": "Solhavn Transport AS",
        "risk_score": 58,
        "naeringskode1": "49.410",
        "naeringskode1_beskrivelse": "Godstransport på vei",
        "kommune": "Trondheim",
        "antall_ansatte": 28,
        "base_revenue": 67_000_000,
        "base_result": 1_100_000,
        "base_assets": 55_000_000,
        "base_equity": 18_000_000,
    ...
    },
    {
        ...
        "navn": "Dalheimfisk AS",
        "risk_score": 40,
        "naeringskode1": "03.111",
        "naeringskode1_beskrivelse": "Havfiske",
        "kommune": "Ålesund",
        "antall_ansatte": 18,
        "base_revenue": 38_000_000,
        "base_result": 4_600_000,
        "base_assets": 72_000_000,
        "base_equity": 31_000_000,
    ...
    },
    {
        ...
        "base_result": 5_400_000,
        "base_assets": 28_000_000,
    ...
    {
        "orgnr": "999100107",
        "navn": "Grønbøl Handel AS",
        "risk_score": 62,
        "naeringskode1": "47.110",
        "naeringskode1_beskrivelse": "Butikkhandel med bredt vareutvalg",
        "kommune": "Kristiansand",
        "antall_ansatte": 45,
        "base_revenue": 118_000_000,
        "base_result": 900_000,
        "base_assets": 48_000_000,
        "base_equity": 11_000_000,
    ...
    {
        "orgnr": "999100108",
        "navn": "Vestfjord Shipping AS",
        "risk_score": 50,
        "naeringskode1": "50.200",
        "naeringskode1_beskrivelse": "Sjøfart langs kysten",
        "kommune": "Haugesund",
        "antall_ansatte": 31,
        "base_revenue": 84_000_000,
        "base_result": 3_200_000,
        "base_assets": 145_000_000,
        "base_equity": 52_000_000,
...
_DEMO_INSURERS = [
    {
        ...
        "org_number": "986529551",
        "contact_name": "Anders Holm",
        "contact_email": "anders.holm@if.no",
        "appetite": ["Eiendom", "Ansvar", "Cyber", "Motor"],
    },
    {
        "name": "Gjensidige Forsikring",
        "org_number": "995568217",
        "contact_name": "Lise Bakke",
        "contact_email": "lise.bakke@gjensidige.no",
        "appetite": ["Eiendom", "Ansvar", "Yrkesskade", "Transport"],
    },
    {
        "name": "Tryg Forsikring",
        "org_number": "991567078",
        "contact_name": "Mats Eriksen",
        "contact_email": "mats.eriksen@tryg.no",
        "appetite": ["Eiendom", "Ansvar", "Marine", "Entreprise"],
    },
    {
        "name": "Fremtind Forsikring",
        "org_number": "921698408",
        "contact_name": "Nina Strand",
        "contact_email": "nina.strand@fremtind.no",
        "appetite": ["Motor", "Eiendom", "Ansvar", "Yrkesskade"],
    },
    {
        "name": "Storebrand Forsikring",
        "org_number": "930553506",
        "contact_name": "Erik Lie",
        "contact_email": "erik.lie@storebrand.no",
        "appetite": ["Styreansvar", "Eiendom", "Ansvar", "Cyber"],
    },
]
...
def _seed_insurers(db: Session, firm_id: int, now: datetime) -> dict[str, int]:
    ...
    for ins in _DEMO_INSURERS:
        existing = (
            db.query(Insurer)
            .filter(Insurer.firm_id == firm_id, Insurer.name == ins["name"])
            .first()
        )
        if existing:
...
def _seed_contacts(db: Session, now: datetime) -> int:
    ...
    for orgnr, name, title, email, is_primary in _DEMO_CONTACTS:
        if (
            ...
            .filter(ContactPerson.orgnr == orgnr, ContactPerson.email == email)
            .first()
        ):
            continue
        db.add(
...
def _seed_submissions(
    db: Session, firm_id: int, insurer_map: dict[str, int], now: datetime
) -> int:
    ...
    for c in _COMPANIES:
        ...
        if (
            db.query(Submission)
            .filter(
                Submission.orgnr == orgnr,
                Submission.firm_id == firm_id,
...
def _seed_company_row(db: Session, c: dict) -> bool:
    ...
    if db.query(Company).filter(Company.orgnr == c["orgnr"]).first():
        return False
    eq_ratio = (
        round(c["base_equity"] / c["base_assets"], 3) if c["base_assets"] else 0.0
    )
    db.add(
        Company(
            orgnr=c["orgnr"],
...
def _seed_history_rows(db: Session, c: dict) -> int:
    ...
    for row in _build_history_rows(c):
        exists = (
            ...
            .filter(
                CompanyHistory.orgnr == c["orgnr"], CompanyHistory.year == row["year"]
            )
            .first()
        )
        if exists:
...
def _seed_primary_policy(
    db: Session, c: dict, firm_id: int, today: date, now: datetime
) -> tuple[int | None, bool]:
...
_DEMO_DEALS = [
    {
        "orgnr": "999100101",
        "title": "Fornye eiendomsforsikring",
        "premium": 969_000,
        "stage_kind": PipelineStageKind.qualified,
        "days_out": 30,
    },
    {
        "orgnr": "999100102",
        "title": "Ny cyberforsikring",
        "premium": 635_000,
        "stage_kind": PipelineStageKind.lead,
        "days_out": 60,
    },
    {
        "orgnr": "999100103",
        "title": "Utvidet entrepriseforsikring",
        "premium": 1_120_000,
        "stage_kind": PipelineStageKind.quoted,
        "days_out": 14,
    },
    {
        "orgnr": "999100104",
        "title": "Fornye transportforsikring",
        "premium": 1_495_000,
        "stage_kind": PipelineStageKind.bound,
        "days_out": 7,
    },
    {
        "orgnr": "999100105",
        "title": "Ny fiskefartøyforsikring",
        "premium": 651_000,
        "stage_kind": PipelineStageKind.lead,
        "days_out": 45,
    },
...
def _seed_demo_deals(db: Session, firm_id: int, now: datetime) -> int:
    ...
    stages = {
        s.kind: s.id
        for s in db.query(PipelineStage).filter(PipelineStage.firm_id == firm_id).all()
    }
    if not stages:
        return 0
    created = 0
    for d in _DEMO_DEALS:
        ...
        db.add(
            Deal(
                ...
                stage_id=stage_id,
                title=d["title"],
                expected_premium_nok=d["premium"],
                expected_close_date=date.today() + timedelta(days=d["days_out"]),
                source="Demo",
                notes="Fiktiv deal for demo",
                created_at=now,
                updated_at=now,
            )
        )
...
class DemoSeedService:
    ...
    def seed_full_demo(self) -> dict:
        ...
        for c in _COMPANIES:
            ...
            policy_id, policy_created = _seed_primary_policy(
                self.db, c, firm_id, today, now
            )
            if policy_created:
                counts["policies"] += 1
        ...
        _seed_demo_deals(self.db, firm_id, now)
