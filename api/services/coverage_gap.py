...
def _build_gap_item(
    need: dict[str, Any], active_policies: list[Policy]
) -> dict[str, Any]:
    ...
    if best:
        ...
        if (
            est_cov
            and best.coverage_amount_nok
            and best.coverage_amount_nok < est_cov * 0.7
        ):
            diff_mnok = (est_cov - best.coverage_amount_nok) / 1_000_000
            coverage_note = (
                f"Mulig underdekning: {diff_mnok:.1f} MNOK under anbefalt nivå"
            )
        return {
            "type": rec_type,
        ...
        for orgnr in orgnrs:
            try:
                ...
                if analysis["gap_count"] > 0:
                    company = (
                        self.db.query(Company).filter(Company.orgnr == orgnr).first()
                    )
                    results.append(
                        {
                            "orgnr": orgnr,
