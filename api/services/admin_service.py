...
_DEMO_CLAIMS_DATA = [
    (
        ...
        "SKD-2025-001",
        ClaimStatus.open,
        "Skade på kontorbygg — vanninntrenging",
        250_000,
    ),
    (
        "POL-EQN-001",
    ...
    (
        ...
        "SKD-2024-018",
        ClaimStatus.settled,
        "Sprinkleranlegg utløst — vannskade",
        380_000,
    ),
    (
        "POL-AKB-001",
...
_DEMO_ACTIVITIES_DATA = [
    ...
    (
        "982463718",
        ActivityType.call,
        "Avklare dekningsomfang Cyberforsikring",
        14,
        False,
    ),
...
class AdminService:
    ...
    def reset(self) -> dict:
        ...
        result = self.db.execute(
            text("DELETE FROM company_chunks WHERE orgnr != 'knowledge'")
        )
    ...
    def seed_norway_top100(self) -> dict:
        ...
        lookup_added, lookup_failed, total_in_portfolio = self._add_top100_companies(
            portfolio.id
        )
        PortfolioService(self.db).enrich_pdfs_background(portfolio.id)
        return {
            ...
            "already_present": total_in_portfolio - lookup_added,
    ...
    def _seed_policies(self, firm_id: int, now: datetime) -> tuple[int, int, dict]:
        ...
        for (
            ...
            days_to_renewal,
            stage,
        ) in _DEMO_POLICIES_DATA:
            exists = (
                self.db.query(Policy)
                .filter(Policy.policy_number == pol_nr, Policy.firm_id == firm_id)
                .first()
            )
            if exists:
            ...
            p = Policy(
                orgnr=orgnr,
                firm_id=firm_id,
                policy_number=pol_nr,
                insurer=insurer,
                product_type=product_type,
                annual_premium_nok=float(premium),
                coverage_amount_nok=float(coverage),
    ...
    def _seed_claims(self, policy_map: dict, firm_id: int, now: datetime) -> int:
        ...
        for pol_nr, orgnr, claim_nr, status, desc, amount in _DEMO_CLAIMS_DATA:
            ...
            if (
                self.db.query(Claim)
                .filter(Claim.claim_number == claim_nr, Claim.firm_id == firm_id)
                .first()
            ):
                continue
            self.db.add(
                Claim(
                    ...
                    orgnr=orgnr,
                    firm_id=firm_id,
                    claim_number=claim_nr,
                    incident_date=today - timedelta(days=30),
                    reported_date=today - timedelta(days=25),
                    status=status,
                    description=desc,
                    estimated_amount_nok=float(amount),
                    created_at=now,
                    updated_at=now,
                )
            )
        ...
        for orgnr, atype, subject, days_delta, completed in _DEMO_ACTIVITIES_DATA:
            ...
            self.db.add(
                Activity(
                    ...
                    activity_type=atype,
                    subject=subject,
                    due_date=today + timedelta(days=days_delta),
                    completed=completed,
