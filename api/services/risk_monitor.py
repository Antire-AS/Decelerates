...
def _update_company_fields(
    company: Company, regn: dict, new_score: int, eq_ratio: float | None
) -> None:
...
def _notify_risk_changes(risk_changes: list[dict], firm_id: int, db: Session) -> None:
    ...
    for ch in risk_changes:
        direction = (
            "økt" if (ch["new_score"] or 0) > (ch["old_score"] or 0) else "redusert"
        )
        create_notification_for_users_safe(
            db,
            firm_id=firm_id,
...
class RiskMonitorService:
    ...
    def refresh_all_portfolios(self, firm_id: int) -> dict[str, Any]:
        ...
        orgnrs = list(
            {pc.orgnr for pc in self.db.query(PortfolioCompany.orgnr).distinct().all()}
        )
        refreshed = 0
