...
def _stream_pdf_phase(pc, navn: str, i: int, total: int, db: Session) -> Iterator[str]:
    ...
    covered = {
        s.year
        for s in db.query(CompanyPdfSource)
        .filter(CompanyPdfSource.orgnr == pc.orgnr)
        .all()
    }
    ...
    try:
        org = fetch_enhet_by_orgnr(pc.orgnr) or {
            "navn": navn,
            "organisasjonsnummer": pc.orgnr,
        }
...
def _stream_ingest_company(
    pc, i: int, total: int, include_pdfs: bool, db: Session
) -> Iterator[str]:
...
class PortfolioStreamingService:
    ...
    def stream_ingest(self, portfolio_id: int, include_pdfs: bool) -> Iterator[str]:
        ...
        total = len(rows)
        yield (
            json.dumps({"type": "start", "total": total, "include_pdfs": include_pdfs})
            + "\n"
        )
    ...
    def stream_seed_norway(self, portfolio_id: int) -> Iterator[str]:
        ...
        for i, name in enumerate(TOP_100_NO_NAMES):
            yield (
                json.dumps(
                    {"type": "searching", "name": name, "index": i + 1, "total": total}
                )
                + "\n"
            )
            try:
    ...
    def stream_batch_import(
        self, portfolio_id: int | None, orgnrs: list[str], invalid_count: int
    ) -> Iterator[str]:
        ...
        total = len(orgnrs)
        yield (
            json.dumps({"type": "start", "total": total, "invalid": invalid_count})
            + "\n"
        )
        added, failed = 0, 0
        for i, orgnr in enumerate(orgnrs):
            ...
            try:
                ...
                navn = (
                    (result or {}).get("org", {}).get("navn", orgnr)
                    if result
                    else orgnr
                )
                if portfolio_id:
...
def stream_batch_import(
    portfolio_id: int | None, orgnrs: list[str], invalid_count: int, db: Session
) -> Iterator[str]:
    return PortfolioStreamingService(db).stream_batch_import(
        portfolio_id, orgnrs, invalid_count
    )
