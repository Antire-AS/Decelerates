...
def _portfolio_cover(
    pdf: Any, portfolio_name: str, broker: dict, generated_at: str
) -> None:
    pdf.add_page()
    ...
    pdf.cell(
        0, 8, f"Generert: {generated_at}", align="C", new_x="LMARGIN", new_y="NEXT"
    )
    if broker.get("contact_name"):
        pdf.cell(
            0,
            8,
            _safe(f"Ansvarlig megler: {broker['contact_name']}"),
            align="C",
            new_x="LMARGIN",
            new_y="NEXT",
        )
...
def _portfolio_concentration_section(pdf: Any, concentration: dict) -> None:
    ...
    pdf.cell(
        0,
        7,
        f"Totalt eksponert omsetning: {total_rev / 1e9:.1f} BNOK",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.ln(2)
    ...
    for row in (concentration.get("by_geography") or [])[:5]:
        pdf.cell(
            0,
            5,
            f"  {_safe(row.get('kommune', ''))}: {row['count']} selskaper",
            new_x="LMARGIN",
            new_y="NEXT",
        )
    pdf.ln(3)
    ...
    for row in concentration.get("by_size") or []:
        pdf.cell(
            0,
            5,
            f"  {_safe(row.get('band', ''))}: {row['count']} selskaper",
            new_x="LMARGIN",
            new_y="NEXT",
        )
