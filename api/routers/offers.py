...
async def compare_offers(
    ...
    if len(files) > _MAX_FILES:
        raise HTTPException(
            status_code=400, detail=f"Maximum {_MAX_FILES} files per request"
        )
    ...
    for f in files:
        if f.content_type not in _ALLOWED_MIME_TYPES:
            raise HTTPException(
                status_code=415, detail=f"{f.filename}: only PDF files are accepted"
            )
        raw = await f.read()
        if len(raw) > _MAX_UPLOAD_BYTES:
            raise HTTPException(
                status_code=413, detail=f"{f.filename}: file exceeds 50 MB limit"
            )
        try:
        ...
        offer_texts.append(
            {
                "name": f.filename or f"Tilbud {len(offer_texts) + 1}",
                "text": text[:10000],
            }
        )
    ...
    comparison = _compare_offers_with_llm(
        _compare_prompt(len(offer_texts), company_ctx, offers_block)
    )
    return {
        "orgnr": orgnr,
...
def list_offers(orgnr: str, db: Session = Depends(get_db)) -> list:
    ...
    rows = (
        ...
        .filter(InsuranceOffer.orgnr == orgnr)
        .order_by(InsuranceOffer.id)
        .all()
    )
    return [
        {
...
def compare_stored_offers(
    ...
    comparison = _compare_offers_with_llm(
        _compare_prompt(len(rows), company_ctx, offers_block)
    )
    return {
        "orgnr": orgnr,
