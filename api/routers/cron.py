...
def _resolve_single_firm_id(db: Session) -> int:
    ...
    if firm_count > 1:
        raise HTTPException(
            status_code=409, detail="Multiple BrokerFirms — refactor to per-firm cron."
        )
...
def _check_recipient(db: Session) -> str:
    ...
    if not recipient:
        raise HTTPException(
            status_code=422, detail="Ingen broker contact_email konfigurert."
        )
    return recipient
...
def send_portfolio_digest(
    ...
    for portfolio in portfolios:
        ...
        results.append(
            {"portfolio": portfolio.name, "alerts": len(alerts), "sent": sent}
        )
...
def send_renewal_threshold_emails(
    ...
    for threshold in [90, 60, 30]:
        ...
        if not policies_needing:
            results.append(
                {"threshold_days": threshold, "policies_found": 0, "sent": False}
            )
            continue
        policy_dicts = [
            {
        ...
        sent = notification.send_renewal_threshold_emails(
            recipient, threshold, policy_dicts
        )
        if sent:
            for p in policies_needing:
        ...
        results.append(
            {
                ...
                "policies_found": len(policies_needing),
                "sent": sent,
            }
        )
...
def send_activity_reminders(
    ...
    overdue = (
        ...
        .filter(
            ...
            Activity.due_date < today,  # noqa: E712
        )
...
def trigger_coverage_gap_alerts(
    ...
    sent = notification.send_email(
        recipient, "Dekningsgap oppdaget i porteføljen", body_html
    )
    log_audit(
        db,
        "coverage_gap_alert_sent",
...
def trigger_renewal_digest(
    ...
    return {
        "recipient": recipient,
        "policies_included": len(renewal_dicts),
        "sent": sent,
    }
