...
def _debug_db_status() -> dict:
    ...
    try:
        with engine.connect() as conn:
            ...
            tags_col = conn.execute(text(
                "SELECT EXISTS (SELECT 1 FROM information_schema.columns "
                "WHERE table_name='insurance_documents' AND column_name='tags')"
            )).scalar()
        return {
            "alembic_version": version, "alembic_error": None,
            "public_tables": [r[0] for r in rows], "tags_column_exists": tags_col,
        }
    except Exception as exc:
        return {"alembic_version": None, "alembic_error": str(exc), "public_tables": [], "tags_column_exists": None}
...
def debug_status() -> dict:
    ...
    return {
        ...
        "blob_count": blob["count"], "blob_sample": blob["sample"], "blob_error": blob["error"],
        "sas_url_works": sas_url_works, "video_info": video_info,
        "alembic_version": db_info["alembic_version"], "alembic_error": db_info["alembic_error"],
        "tags_column_exists": db_info["tags_column_exists"], "public_tables": db_info["public_tables"],
    }
