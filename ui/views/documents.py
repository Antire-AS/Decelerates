...
def _render_video_player(vid: dict) -> None:
    """Render an HTML5 video player with chapter buttons for one video."""
    ...
    height = 530 + max(0, (len(sections) - 3) * 28) if sections else 500
    css = (
...
def render_documents_tab() -> None:
    ...
    docs_sub_lib, docs_sub_vid, docs_sub_cmp = st.tabs(["Dokumentbibliotek", "Videoer", "Sammenlign vilkår"])
    ...
    with docs_sub_vid:
        ...
        try:
            videos_resp = requests.get(f"{API_BASE}/videos", timeout=15)
            videos = videos_resp.json() if videos_resp.ok else []
        except Exception:
            videos = []

        if not videos:
            st.info("Ingen videoer lastet opp ennå.")
        else:
            for vid in videos:
                _render_video_player(vid)
