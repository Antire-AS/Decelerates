...
def test_reasons_matches_factor_labels():
    result = derive_simple_risk(_org(naeringskode1="41.20"),
                                _regn(sum_driftsinntekter=200_000_000))
...
def test_score_equals_sum_of_factor_points():
    result = derive_simple_risk(
        ...
        _regn(sum_driftsinntekter=200_000_000,
              sum_egenkapital=-500_000, sum_eiendeler=5_000_000),
    )
...
def test_negative_equity_adds_2():
    result = derive_simple_risk(_org(),
                                _regn(sum_egenkapital=-1_000_000, sum_eiendeler=10_000_000))
...
def test_healthy_equity_no_score():
    # 40% > 20% threshold
    result = derive_simple_risk(_org(),
                                _regn(sum_egenkapital=4_000_000, sum_eiendeler=10_000_000))
    assert not any("egenkapital" in f["label"].lower() for f in result["factors"])


def test_equity_ratio_calculated_correctly():
    result = derive_simple_risk(_org(),
                                _regn(sum_egenkapital=3_000_000, sum_eiendeler=10_000_000))
...
def test_negative_aarsresultat_adds_1():
    ...
    assert any(f["category"] == "Økonomi" and "årsresultat" in f["label"].lower()
               for f in result["factors"])
...
def test_very_high_debt_over_80pct_adds_2():
    result = derive_simple_risk(_org(),
                                _regn(sum_gjeld=9_000_000, sum_eiendeler=10_000_000))
    assert any(f["points"] == 2 and "gjeldsgrad" in f["label"].lower()
               for f in result["factors"])


def test_high_debt_60_to_80pct_adds_1():
    result = derive_simple_risk(_org(),
                                _regn(sum_gjeld=7_000_000, sum_eiendeler=10_000_000))
    assert any(f["points"] == 1 and "gjeldsgrad" in f["label"].lower()
               for f in result["factors"])


def test_low_debt_no_score():
    result = derive_simple_risk(_org(),
                                _regn(sum_gjeld=3_000_000, sum_eiendeler=10_000_000))
...
@pytest.mark.parametrize("nace", ["41.20", "42.10", "43.21", "49.10", "52.10", "64.11", "77.10"])
def test_high_risk_nace_sections_add_2(nace):
    result = derive_simple_risk(_org(naeringskode1=nace), _regn())
    assert any(f["points"] == 2 and f["category"] == "Bransje" for f in result["factors"]), \
...
def test_med_risk_nace_sections_add_1(nace):
    result = derive_simple_risk(_org(naeringskode1=nace), _regn())
    assert any(f["points"] == 1 and f["category"] == "Bransje" for f in result["factors"]), \
...
def test_company_2_to_5_years_adds_1():
    ...
    assert any(f["points"] == 1 and f["category"] == "Historikk" for f in result["factors"])
...
def test_build_risk_summary_maps_fields():
    ...
    regn = _regn(sum_driftsinntekter=50_000_000, aarsresultat=1_000_000, antall_ansatte=80)
    risk = {"score": 5, "reasons": ["Høy omsetning"], "factors": [], "equity_ratio": 0.25}
    pep = {"hit_count": 0}
