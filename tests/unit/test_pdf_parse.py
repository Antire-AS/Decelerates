...
def test_download_pdf_bytes_returns_none_on_unexpected_exception():
    with patch(
        "api.services.pdf_parse.requests.get", side_effect=ConnectionError("no route")
    ):
...
@patch(
    "api.services.pdf_parse._parse_json_financials",
...
@patch(
    "api.services.pdf_parse._parse_json_financials",
    return_value={"revenue": 500_000, "equity_ratio": 0.1},
)
@patch("api.services.pdf_parse._try_gemini", return_value='{"revenue": 500000}')
def test_try_gemini_with_retry_retries_on_sanity_fail(
    mock_try, mock_parse, mock_sanity
):
    result = _try_gemini_with_retry(b"pdf", "123", 2023)
    assert result is not None
...
@patch(
    "api.services.pdf_parse._parse_json_financials", return_value={"revenue": 500_000}
)
@patch("api.services.pdf_parse._try_gemini", return_value='{"revenue": 500000}')
def test_try_gemini_with_retry_returns_none_when_both_attempts_fail_sanity(
    mock_try, mock_parse, mock_sanity
):
    result = _try_gemini_with_retry(b"pdf", "123", 2023)
    assert result is None
...
@patch(
    "api.services.pdf_parse._try_gemini_with_retry",
    return_value={"revenue": 1_000_000, "equity_ratio": 0.3},
)
