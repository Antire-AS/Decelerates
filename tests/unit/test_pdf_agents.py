...
def test_run_tool_web_search_uses_gemini_then_ddg_fallback():
    ...
    with patch("api.services.pdf_agents._gemini_web_search", return_value=[]), \
         patch("api.services.pdf_agents._ddg_search_results", return_value=[{"url": "http://x"}]):
...
def test_run_tool_web_search_returns_gemini_results_first():
    ...
    with patch("api.services.pdf_agents._gemini_web_search", return_value=gemini_results):
        result = _run_tool("web_search", {"query": "test"})
...
def test_run_tool_fetch_url_delegates():
    from api.services.pdf_agents import _run_tool
    with patch("api.services.pdf_agents._fetch_url_content", return_value={"text": "hi"}):
...
def test_agent_system_prompt_contains_company_info():
    ...
    prompt = _agent_system_prompt("TestCo", "123456789", "https://testco.no", [2023, 2024])
...
def test_claude_agent_returns_pdf_links(monkeypatch):
    ...
    with patch("anthropic.Anthropic", return_value=mock_client), \
        ...
        result = _agent_discover_pdfs_claude("123456789", "Test AS", "https://example.com", [2023], "test-key")
    assert len(result) == 1
...
def test_claude_agent_calls_fetch_url_tool(monkeypatch):
    ...
    with patch("anthropic.Anthropic", return_value=mock_client), \
         patch("api.services.pdf_agents._fetch_url_content", return_value=fetch_result) as mock_fetch:
        from api.services.pdf_agents import _agent_discover_pdfs_claude
...
def test_gemini_web_search_returns_results():
    ...
    web_chunk = SimpleNamespace(web=SimpleNamespace(uri="https://x.com/ir", title="IR Page"))
    ...
    with patch("api.services.pdf_agents._gemini_api_keys", return_value=["fake-key"]), \
...
def test_gemini_web_search_handles_quota_error():
    ...
    with patch("api.services.pdf_agents._gemini_api_keys", return_value=["key1"]), \
         patch("api.services.pdf_agents.google_genai.Client", return_value=mock_client):
        assert _gemini_web_search("q") == []
...
def test_gemini_web_search_returns_empty_when_no_candidates():
    ...
    with patch("api.services.pdf_agents._gemini_api_keys", return_value=["k"]), \
         patch("api.services.pdf_agents.google_genai.Client", return_value=mock_client):
        assert _gemini_web_search("q") == []
...
def test_run_gemini_phase2_returns_text_when_no_tool_calls():
    ...
    part = SimpleNamespace(text='[{"year": 2023, "pdf_url": "https://x.com/a.pdf", "label": "AR"}]', function_call=None)
...
def test_run_gemini_phase2_handles_tool_call_loop():
    ...
    resp1 = SimpleNamespace(candidates=[SimpleNamespace(content=SimpleNamespace(parts=[part1]))])

    part2 = SimpleNamespace(text='[{"year": 2023, "pdf_url": "https://x.com/a.pdf", "label": "AR"}]', function_call=None)
    resp2 = SimpleNamespace(candidates=[SimpleNamespace(content=SimpleNamespace(parts=[part2]))])

    mock_chat = MagicMock()
    mock_chat.send_message.side_effect = [resp1, resp2]

    with patch("api.services.pdf_agents._run_tool", return_value={"text": "page content", "pdf_links": []}):
...
def test_gemini_agent_returns_results():
    ...
    part = SimpleNamespace(text='[{"year": 2023, "pdf_url": "https://x.com/a.pdf", "label": "AR"}]', function_call=None)
    chat_resp = SimpleNamespace(candidates=[SimpleNamespace(content=SimpleNamespace(parts=[part]))])
    mock_chat = MagicMock()
...
def test_gemini_agent_uses_homepage_fallback_when_phase1_empty():
    ...
    with patch("api.services.pdf_agents.google_genai.Client", return_value=mock_client):
        result = _agent_discover_pdfs_gemini("123", "Co", "https://co.no", [2023], "key")
    assert result == []
...
def test_search_pdf_url_for_year_falls_back_to_ddg():
    ...
    mock_client = MagicMock()
    mock_client.models.generate_content.return_value = response
    with patch("api.services.pdf_agents._ddg_query", return_value=["https://ddg.com/ar.pdf"]):
        result = _search_pdf_url_for_year(mock_client, "TestCo", None, 2023)
...
def test_azure_openai_agent_parses_stop_response():
    from api.services.pdf_agents import _agent_discover_pdfs_azure_openai
    pdf_json = json.dumps([{"year": 2023, "pdf_url": "https://x.com/a.pdf", "label": "AR"}])
    msg = SimpleNamespace(content=pdf_json, tool_calls=None)
    msg.model_dump = lambda exclude_unset=False: {"role": "assistant", "content": pdf_json}
    ...
    with patch.dict("os.environ", {"AZURE_OPENAI_ENDPOINT": "https://oai.azure.com", "AZURE_OPENAI_API_KEY": "key"}), \
...
def test_azure_openai_agent_handles_tool_call_then_stop():
    ...
    tc = SimpleNamespace(id="tc_1", function=SimpleNamespace(name="web_search", arguments='{"query":"test"}'))
    msg1 = SimpleNamespace(content=None, tool_calls=[tc])
    msg1.model_dump = lambda exclude_unset=False: {"role": "assistant", "tool_calls": [{"id": "tc_1"}]}
    choice1 = SimpleNamespace(message=msg1, finish_reason="tool_calls")
    resp1 = SimpleNamespace(choices=[choice1])

    pdf_json = json.dumps([{"year": 2024, "pdf_url": "https://x.com/b.pdf", "label": "AR"}])
    msg2 = SimpleNamespace(content=pdf_json, tool_calls=None)
    msg2.model_dump = lambda exclude_unset=False: {"role": "assistant", "content": pdf_json}
...
def test_orchestrator_tries_gemini_agent_after_per_year_empty():
    ...
    with patch.dict("os.environ", {}, clear=True), \
         patch("api.services.pdf_agents._agent_discover_pdfs_azure_openai", return_value=[]), \
         patch("api.services.pdf_agents._gemini_api_keys", return_value=["gkey"]), \
         patch("api.services.pdf_agents._discover_pdfs_per_year_search", return_value=[]), \
         patch("api.services.pdf_agents._agent_discover_pdfs_gemini", return_value=expected):
        result = _agent_discover_pdfs("123", "TestCo", None, [2023])
    assert result == expected
...
def test_orchestrator_skips_claude_when_key_is_placeholder():
    ...
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "your_key_here"}), \
         patch("api.services.pdf_agents._agent_discover_pdfs_azure_openai", return_value=[]) as m_az, \
         patch("api.services.pdf_agents._gemini_api_keys", return_value=[]):
...
def test_discover_ir_pdfs_parses_llm_response():
    from api.services.pdf_agents import _discover_ir_pdfs
    llm_resp = json.dumps([{"year": 2023, "pdf_url": "https://x.com/a.pdf", "label": "AR"}])
    with patch("api.services.pdf_agents._search_all_annual_pdfs", return_value=["https://x.com/a.pdf"]), \
...
def test_discover_ir_pdfs_handles_malformed_json_with_regex():
    ...
    with patch("api.services.pdf_agents._search_all_annual_pdfs", return_value=["https://x.com/b.pdf"]), \
