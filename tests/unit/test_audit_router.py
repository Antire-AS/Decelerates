...
def test_get_audit_log_global_returns_paginated():
    ...
    db = _mock_db(rows)
    result = get_audit_log_global(
        orgnr=None, limit=100, db=db, user=_mock_user(), **_PAGE_KW
    )
...
def test_get_audit_log_orgnr_returns_paginated():
    ...
    result = get_audit_log(
        orgnr="987654321", limit=50, db=db, user=_mock_user(), **_PAGE_KW
    )
    assert len(result["items"]) == 2
...
def test_get_audit_log_orgnr_shape():
    db = _mock_db([_audit_row()])
    result = get_audit_log(
        orgnr="123456789", limit=50, db=db, user=_mock_user(), **_PAGE_KW
    )
    entry = result["items"][0]
    assert "id" in entry
    assert "action" in entry
...
def test_get_audit_log_orgnr_empty():
    db = _mock_db([])
    result = get_audit_log(
        orgnr="000000000", limit=50, db=db, user=_mock_user(), **_PAGE_KW
    )
    assert result["items"] == []
    assert result["total"] == 0
