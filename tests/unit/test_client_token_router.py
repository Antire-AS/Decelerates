...
def test_create_token_returns_200(client, mock_db):
    mock_db.query.return_value.filter.return_value.first.return_value = MagicMock()  # company exists
    token_row = SimpleNamespace(token="abc123")
    with patch("api.routers.client_token.create_token", return_value=token_row), \
...
def test_list_tokens_returns_list(client):
    ...
    rows = [
        SimpleNamespace(token="tok1", label="L1",
...
def test_get_client_profile_returns_200(client, mock_db):
    ...
    token_row = SimpleNamespace(
        token="valid-tok", orgnr="123456789", label="Test",
    ...
    profile = {"org": {"navn": "Test AS", "kommune": "Oslo"}, "risk": {"score": 3, "reasons": []}, "regnskap": {}}

    with patch("api.routers.client_token.fetch_org_profile", return_value=profile), \
         patch("api.routers.client_token.log_audit"), \
         patch("api.routers.client_token._fetch_crm_snapshot",
...
def test_get_client_profile_returns_410_for_expired_token(client, mock_db):
    expired_row = SimpleNamespace(
        token="expired-tok", orgnr="123456789", label="Old",
