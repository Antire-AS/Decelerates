...
def test_pep_screen_returns_hits_from_results_field():
    fake = {
        "results": [
            {"id": "Q1", "name": "John Doe", "schema": "Person",
             "datasets": ["sanctions"], "topics": ["sanction"]},
            {"id": "Q2", "name": "John Doerson", "schema": "Person",
             "datasets": [], "topics": []},
        ]
    }
    with patch("api.services.screening_client.requests.get",
               return_value=_mock_resp(200, fake)):
        ...
        "entities": [
            {
                ...
                "licenses": [
                    {
                        "id": "L1", "type": "Banking", "status": "Active",
                        "validFrom": "2020-01-01", "validTo": None,
                        "description": "Bankvirksomhet",
                    },
                    {
                        "id": "L2", "type": "Investment", "status": "Active",
                        "validFrom": "2021-01-01", "validTo": None,
...
def test_fetch_losore_caps_pledge_list_at_10():
    fake_pledges = [{"dokumentnummer": f"D{i}", "innkomsttidspunkt": "2024-01-01T00:00:00"}
                    for i in range(20)]
