...
def seed_broker_firm(test_db):
    ...
    if not test_db.query(BrokerFirm).filter(BrokerFirm.id == _FIRM_ID).first():
        test_db.add(BrokerFirm(
            id=_FIRM_ID, name="Deal Test Firma AS",
...
class TestPipelineStages:
    ...
    def test_create_stage_returns_201(self, auth_client):
        resp = auth_client.post("/pipeline/stages", json={
            "name": "Nytt lead",
            "kind": "lead",
            "order_index": 0,
            "color": "#4A6FA5",
    ...
    def test_create_duplicate_name_fails(self, auth_client):
        auth_client.post("/pipeline/stages", json={
            "name": "Duplikat", "kind": "lead", "order_index": 0,
        })
        resp = auth_client.post("/pipeline/stages", json={
            "name": "Duplikat", "kind": "qualified", "order_index": 1,
        })
...
def _create_stage(auth_client, name="Test stage", kind="lead") -> int:
    ...
    resp = auth_client.post("/pipeline/stages", json={
        "name": name, "kind": kind, "order_index": 0,
    })
...
class TestDeals:
    ...
    def test_list_deals_includes_created(self, auth_client):
        ...
        auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": stage_id, "title": "Listbar deal",
        })
    ...
    def test_list_deals_filter_by_orgnr(self, auth_client):
        ...
        auth_client.post("/deals", json={
            "orgnr": "111222333", "stage_id": stage_id, "title": "Other org",
        })
        auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": stage_id, "title": "Target org",
        })
    ...
    def test_update_deal(self, auth_client):
        ...
        deal_id = auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": stage_id, "title": "Gammel tittel",
        }).json()["id"]

        resp = auth_client.patch(f"/deals/{deal_id}", json={
            "title": "Ny tittel",
    ...
    def test_move_deal_to_stage(self, auth_client):
        ...
        deal_id = auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": stage1, "title": "Flyttes",
        }).json()["id"]

        resp = auth_client.patch(f"/deals/{deal_id}/stage", json={
            "stage_id": stage2,
        })
        assert resp.status_code == 200
    ...
    def test_lose_deal_sets_lost_fields(self, auth_client):
        ...
        deal_id = auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": stage_id, "title": "Tapt deal",
        }).json()["id"]

        resp = auth_client.post(f"/deals/{deal_id}/lose", json={
    ...
    def test_delete_deal(self, auth_client):
        ...
        deal_id = auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": stage_id, "title": "Slettes",
        }).json()["id"]
    ...
    def test_create_deal_with_invalid_stage_returns_400(self, auth_client):
        resp = auth_client.post("/deals", json={
            "orgnr": _ORGNR, "stage_id": 999999, "title": "Bad stage",
        })
        assert resp.status_code == 400
